...
5. 知识库资料只用于辅助生成，不要在正文中显示来源编号、引用编号、脚注标记或类似 [S1] 的标记。
...
- 不输出来源编号、引用编号、脚注标记或类似 [S1]、[S2] 的内容。
...
【条件优先级规则】
- 如果原始需求里的城市、时间、身份、用途或输出风格与结构化条件冲突，必须以结构化条件为准。
- 不得因为原始需求中残留旧城市或旧时长，就覆盖用户刚刚在页面中选择的新条件。
...
【事实使用要求】
- 优先使用检索资料中的信息。
- 知识库资料只用于事实核验和内容生成，不要在最终正文显示来源编号、引用编号或脚注标记。
...
- 知识库资料只用于事实核验和内容生成，不要在最终正文显示来源编号、引用编号、脚注标记或类似 [S1] 的内容。
