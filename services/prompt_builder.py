...
TASK_INSTRUCTIONS: dict[TaskType, str] = {
    TaskType.QA: """
【任务模式：非遗知识问答】
建议结构：一句话解释、文化背景、核心看点、体验方式、核验提醒。
重点是讲清楚、讲得懂，并把知识延伸到真实体验。
""".strip(),
    TaskType.ROUTE: """
【任务模式：岭南文化路线】
建议结构：方案总览、行程时间轴、节点详情、体验与记录建议、出发前提醒。
路线要考虑节奏和地点关系；没有地理或实时证据时，不要声称具体通勤时长。
""".strip(),
    TaskType.STUDY: """
【任务模式：研学任务】
建议结构：研学主题、学习目标、行前准备、现场任务卡、采访问题、报告提纲。
任务必须可观察、可记录、可形成结论，避免只让学生抄资料。
""".strip(),
    TaskType.SOCIAL: """
【任务模式：图文传播】
建议结构：内容定位、标题候选、完整正文、配图建议、标签、可延展选题。
成品要可以直接发布，但不能把未经核验的细节包装成事实。
""".strip(),
    TaskType.VIDEO: """
【任务模式：短视频脚本】
建议结构：视频定位、3 秒钩子、60 秒分镜表、旁白字幕、拍摄建议、标题与标签。
镜头与旁白要可执行，避免空洞口号。
""".strip(),
}
