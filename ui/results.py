...
from core.models import MAX_REVISION_INSTRUCTION_CHARS, TaskRequest, TaskType
...
def _render_sources(sources: str) -> None:
    if sources.strip():
        display_sources = _SOURCE_MARKER_PATTERN.sub("", sources)
        st.markdown(display_sources)
        st.caption(
            "这里展示本次命中的项目知识库资料；实时开放、票务和演出信息仍请以官方平台最新公告为准。"
        )
    else:
        st.info("当前结果没有可展示的检索资料记录。")
...
def _render_revision_panel(request: TaskRequest) -> None:
    with st.container(border=True):
        ...
        custom_instruction = st.text_area(
            ...
            key="custom_revision",
            height=95,
            max_chars=MAX_REVISION_INSTRUCTION_CHARS,
